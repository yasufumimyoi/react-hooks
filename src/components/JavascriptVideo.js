...
//
const JavascriptVideo = (props) => {
  //
  const { JVideo, setJVideo, currentUser, guestUser } = useContext(
    VideoContext
  );
  ...
  //
  const matchedVideo = JVideo.filter((video) => video.id === parsedId);

  //
  //動画視聴後にこのメソッドが呼ばれてcompletedにチェックが付く
  const saveCompletedStatus = (id) => {
    const newItems = JVideo.map((item) => {
      if (item.id === id) {
    ...
    if (currentUser.isAnonymous === false || guestUser.isAnonymous === false) {
      firestore
        ...
        .doc(currentUser.uid)
        //
        .update({ javascript: [...newItems] });
    } else {
      console.log('Guest user data updated');
      sessionStorage.setItem('javascript', JSON.stringify(newItems));
    }
  };
